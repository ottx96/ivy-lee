...
import com.github.ottx96.ivy.ui.dialog.impl.TimeParser
...
class TaskDialog : View("Task Dialog"){
    ...
    init {
        ...
        File(Entrypoint::class.java.classLoader.getResource("extension/extensions.txt")!!.file).useLines { lines ->
            lines.filter(String::isNotBlank).forEach { line ->
                try{
                    val cl = Class.forName(line)
    ...
    private fun updatePriorityIndicators(priority: Priorities){
        ...
        prioritiesMapping.entries.reversed().forEach {
            ...
            else it.component1().image = Image("/images/priority_element.png")
        }
    }
